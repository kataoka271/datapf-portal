...
- [ ] **`catalog_roles` always empty in prod** (`portal/backend/app/auth.py:51-57`)
...
- [ ] **`get_catalog` has no prod branch** (`portal/backend/app/routers.py:152-153`)
...
- [ ] **`get_members` has no prod branch** (`portal/backend/app/routers.py:229-238`)
...
- [ ] **`create_access_request` reads mock for `requires_approval` in all modes** (`portal/backend/app/routers.py:288-290`)
...
- [ ] **`subscribe_app` SQL syntax error** (`portal/backend/app/routers.py:470`)
  `ACTIVE` is unquoted in the INSERT VALUES clause — must be `'ACTIVE'`. Will cause a SQL parse error in prod.

- [ ] **`create_unity_catalog` doesn't assign owner group or grant privileges** (`portal/backend/app/services/databricks.py:126-140`)
...
- [ ] **Notifications not sent on access request** (`portal/backend/app/routers.py:275-317`)
...
- [ ] **Notifications not sent on approval / rejection** (`portal/backend/app/routers.py:320-347`)
  Per spec: notify the applicant on both APPROVED and REJECTED decisions. No notification or email is sent.

- [ ] **App subscription notification missing** (`portal/backend/app/routers.py:459-473`)
...
- [ ] **`list_notifications` double-pagination in prod** (`portal/backend/app/routers.py:56-71`)
...
- [ ] **`mark_all_read` always returns `updated_count: 0`** (`portal/backend/app/routers.py:97`)
  Hardcoded in both dev and prod. Return the actual row count affected.

- [ ] **`send_notification` uses untyped `dict` body** (`portal/backend/app/routers.py:637-642`)
...
- [ ] **`VideoInfo` missing `video_key` field** (`portal/frontend/src/types/index.ts:171-177`)
  Backend returns `video_key: str` per spec, but the frontend type omits it.

- [ ] **`NotificationType` has extra `REVOCATION` value** (`portal/frontend/src/types/index.ts:117`)
...
- [ ] **`cross_search` prod implementation** (`portal/backend/app/routers.py:375-378`)
  Databricks Vector Search API not connected — returns mock in both modes.

- [ ] **`get_statistics` prod SQL** (`portal/backend/app/routers.py:557-560`)
  `PERCENTILE_CONT` / `STDDEV` / `WIDTH_BUCKET` query not written.

- [ ] **`get_vehicle_status` prod SQL** (`portal/backend/app/routers.py:517-523`)
  Spec requires dynamic column fetch from Unity Catalog tags + `system.information_schema`.

- [ ] **`get_vehicle_timeseries` prod SQL** (`portal/backend/app/routers.py:526-533`)
  Spec requires dynamic JOIN query with auto-downsampling above 10,000 rows.

- [ ] **JWKS caching in prod auth** (`portal/backend/app/auth.py:41-43`)
