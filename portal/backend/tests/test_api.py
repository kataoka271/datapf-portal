...
def test_list_catalogs_search():
    ...
    data = res.json()
    assert all(
        "vehicle" in i["catalog_name"].lower()
        or "vehicle" in i.get("description", "").lower()
        for i in data["items"]
    )
...
def test_decide_access_request():
    ...
    res = client.patch(
        "/v1/catalogs/vehicle_timeseries/access-requests/agr-001", json=payload
    )
    assert res.status_code == 200
    data = res.json()
...
def test_decide_access_request_reject():
    ...
    res = client.patch(
        "/v1/catalogs/vehicle_timeseries/access-requests/agr-002", json=payload
    )
    assert res.status_code == 200
    data = res.json()
...
def test_get_vehicle_status():
    res = client.get(
        "/v1/analysis/vehicles/VH-0001/status?at_time=2025-04-09T12:00:00Z"
    )
    assert res.status_code == 200
    data = res.json()
