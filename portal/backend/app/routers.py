...
def list_catalogs(
    ...
    if q:
        ...
        items = [
            i
            for i in items
            if q_lower in i["catalog_name"].lower()
...
def create_catalog(
    body: CreateCatalogRequest, user: CurrentUser = Depends(get_current_user)
):
    settings = get_settings()
    if not settings.dev_mode:
...
def delete_member(
    catalog_name: str, user_id: str, user: CurrentUser = Depends(get_current_user)
):
    settings = get_settings()
    if not settings.dev_mode:
        db_svc.revoke_catalog_viewer(catalog_name, user_id)
...
def get_statistics(
    body: StatisticsRequest, user: CurrentUser = Depends(get_current_user)
):
    settings = get_settings()
    if not settings.dev_mode:
