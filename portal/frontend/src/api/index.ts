...
export const notificationsApi = {
  ...
  markRead: (id: string) =>
    patch<{ notification_id: string; is_read: boolean }>(
      `/notifications/${id}/read`,
    ),
  markAllRead: () =>
    patch<{ updated_count: number }>("/notifications/read-all"),
};
...
export const catalogsApi = {
  list: (params?: {
    ...
    if (params?.subscribed !== undefined)
      q.set("subscribed", String(params.subscribed));
    if (params?.limit) q.set("limit", String(params.limit));
    if (params?.offset) q.set("offset", String(params.offset));
  ...
  updateMou: (
    ...
    put<{ catalog_name: string; version: string; updated_at: string }>(
      `/catalogs/${name}/mou`,
      body,
    ),
  ...
  updateMember: (name: string, userId: string, role: string) =>
    patch<{ user_id: string; role: string }>(
      `/catalogs/${name}/members/${userId}`,
      { role },
    ),
  ...
  getAccessRequests: (name: string, status?: string) => {
    ...
    return get<PaginatedResponse<AccessRequest>>(
      `/catalogs/${name}/access-requests${q}`,
    );
  },
  createAccessRequest: (
    name: string,
    ...
    post<{ agreement_id: string; status: string; role_granted: string | null }>(
      `/catalogs/${name}/access-requests`,
      body,
    ),
  decideAccessRequest: (
    name: string,
    ...
    patch<{ agreement_id: string; status: string; decided_at: string }>(
      `/catalogs/${name}/access-requests/${id}`,
      body,
    ),
...
export const appsApi = {
  ...
  redirectToken: (id: string) =>
    post<{ redirect_url: string; expires_in: number }>(
      `/apps/${id}/redirect-token`,
      {},
    ),
};
...
export const analysisApi = {
  ...
  vehicleStatus: (id: string, atTime: string) =>
    get<VehicleStatus>(
      `/analysis/vehicles/${id}/status?at_time=${encodeURIComponent(atTime)}`,
    ),
  vehicleTimeseries: (
    id: string,
    body: {
      ...
      time_from: string;
      time_to: string;
      downsample_interval_sec?: number;
    },
  ) =>
    post<{ vehicle_id: string; series: TimeseriesSeries[] }>(
      `/analysis/vehicles/${id}/timeseries`,
      body,
    ),
...
export const alertsApi = {
  list: (params?: {
    ...
    if (params?.limit) q.set("limit", String(params.limit));
    if (params?.offset) q.set("offset", String(params.offset));
    return get<PaginatedResponse<QualityAlert>>(
      `/alerts${q.size ? "?" + q : ""}`,
    );
  },
...
export const adminApi = {
  listUsers: (params?: { q?: string; limit?: number; offset?: number }) => {
    ...
    if (params?.limit) q.set("limit", String(params.limit));
    if (params?.offset) q.set("offset", String(params.offset));
    return get<PaginatedResponse<AdminUser>>(
      `/admin/users${q.size ? "?" + q : ""}`,
    );
  },
  sendNotification: (body: {
    ...
    post<{ notification_ids: string[]; recipient_count: number }>(
      "/admin/notifications",
      body,
    ),
};
