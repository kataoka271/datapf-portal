  ...
  return useMutation({
    ...
    onSuccess: (_data, { catalogName }) => {
      qc.invalidateQueries({
        queryKey: ["catalogs", catalogName, "access-requests"],
      });
    },
  });
}
...
export function useVehicleTimeseries(
  ...
  return useQuery({
    ...
    queryFn: () =>
      analysisApi.vehicleTimeseries(vehicleId!, {
        columns,
        time_from: timeFrom,
        time_to: timeTo,
      }),
...
export function useVehicleVideo(
  vehicleId: string | null,
  atTime: string,
  enabled: boolean,
) {
