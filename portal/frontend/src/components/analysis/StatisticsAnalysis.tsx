...
// ── Stat card ─────────────────────────────────────────────────────────────────
function StatCard({ stat, color, onRemove }: { stat: StatResult; color: string; onRemove: () => void }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-xs font-medium text-gray-900 font-mono">{stat.display_name}</p>
          <p className="text-xs text-gray-400 font-mono truncate max-w-xs">{stat.column_full_name}</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: color }} />
          <button onClick={onRemove} className="text-gray-300 hover:text-gray-500 text-lg leading-none">×</button>
        </div>
      </div>
        ...
        {(['histogram', 'stats'] as const).map((t) => (
          <button
            ...
            className={`px-2.5 py-1 text-xs rounded-md border transition-colors ${
              tab === t ? 'bg-teal-600 text-white border-teal-700' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'
            }`}
          >
...
function ColumnSelector({ onSelect }: { onSelect: (col: MatchedColumn) => void }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2">
        ...
        />
        <Button size="sm" variant="primary" onClick={() => query && search.mutate({ query })} loading={search.isPending}>
          検索
        </Button>
      </div>
      {search.isPending && <div className="flex justify-center py-4"><Spinner /></div>}
...
export function StatisticsAnalysis() {
  ...
  const addColumn = (col: MatchedColumn) => {
    ...
    if (!columns.find((c) => `${c.catalog_name}.${c.schema_name}.${c.table_name}.${c.column_name}` === key)) {
      setColumns((prev) => [...prev, col])
    }
    setSelectorOpen(false)
  }

  const removeColumn = (i: number) => setColumns((prev) => prev.filter((_, j) => j !== i))

  const handleRun = () => {
    if (columns.length === 0) { addToast({ type: 'error', message: 'カラムを追加してください' }); return }
    stats.mutate({
      region,
      time_from: new Date(timeFrom).toISOString(),
      time_to:   new Date(timeTo).toISOString(),
      columns: columns.map((c) => `${c.catalog_name}.${c.schema_name}.${c.table_name}.${c.column_name}`),
    }, {
      onError: () => addToast({ type: 'error', message: '統計分析に失敗しました' }),
    })
  }
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <div className="flex gap-1.5 ml-auto">
          <Button size="sm" variant="secondary" onClick={() => setSelectorOpen(!selectorOpen)}>
            + カラム追加
          </Button>
          <Button size="sm" variant="primary" onClick={handleRun} loading={stats.isPending}>
            分析実行
          </Button>
        </div>
      </div>
      ...
      {selectorOpen && (
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-xs font-medium text-gray-700 mb-3">分析対象カラムを選択</p>
      ...
      {columns.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {columns.map((c, i) => (
            <span key={i} className="flex items-center gap-1 px-2.5 py-1 bg-teal-50 border border-teal-200 rounded-full text-xs text-teal-800 font-mono">
              {c.column_name}
              <button onClick={() => removeColumn(i)} className="text-teal-400 hover:text-teal-700 ml-0.5">×</button>
            </span>
          ))}
        </div>
      )}
      ...
      {stats.isPending ? (
        ...
        columns.length === 0 ? (
          <EmptyState title="カラムを追加して分析を実行してください" description="「カラム追加」ボタンから横断検索でカラムを選択できます" />
        ) : (
