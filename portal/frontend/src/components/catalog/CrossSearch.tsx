...
export function CrossSearch() {
  ...
  const selectedCols: MatchedColumn[] = (
    search.data?.matched_columns ?? []
    ...
    saveView.mutate(
      {
        ...
        matched_columns: selectedCols,
      },
      {
        onSuccess: (v) => {
          addToast({
            type: "success",
            message: `VIEW を保存しました: ${v.view_full_name}`,
          });
          setSaveOpen(false);
        },
        onError: () =>
          addToast({ type: "error", message: "VIEW の保存に失敗しました" }),
      },
    );
  };
  ...
  return (
    <div className="p-6">
      <PageHeader
        title="横断検索"
        description="自然言語でカラムを検索し、時系列データを結合します"
      />
        ...
        <div className="grid grid-cols-5 gap-4">
            ...
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs font-medium text-gray-700">
                ...
                <span className="text-gray-400 font-normal">
                  {search.data.matched_columns.length}件
                </span>
              </p>
              {selected.size > 0 && (
                <Button
          ...
          <div className="col-span-3">
            <p className="text-xs font-medium text-gray-700 mb-2">
              時系列プレビュー{" "}
              <span className="text-gray-400 font-normal">
                {search.data.preview_rows.length}行
              </span>
            </p>
                  ...
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      {Object.keys(search.data.preview_rows[0]).map((k) => (
                        <th
                          key={k}
                          className="px-3 py-2 text-left font-medium text-gray-500 font-mono truncate"
                        >
                          {k}
                        </th>
                      ))}
          ...
          <div className="bg-white rounded-xl shadow-xl p-5 w-full max-w-sm">
            ...
            <div className="flex gap-2 justify-end mt-4">
              ...
              <Button
                ...
                disabled={!viewName || !targetSchema}
              >
                保存
              </Button>
            </div>
