...
export function MouEditor({ catalogName }: { catalogName: string }) {
  ...
  const updateItem = (i: number, field: 'label' | 'required', value: string | boolean) =>
    setChecklist((p) => p.map((item, j) => j === i ? { ...item, [field]: value } : item))
  ...
  const handleSave = () => {
    updateMou.mutate(
      ...
      {
        onSuccess: (res) => {
          addToast({ type: 'success', message: `MOU ${res.version} を発行しました` })
          setConfirmOpen(false)
        },
        onError: () => addToast({ type: 'error', message: '更新に失敗しました' }),
      }
    )
  }
  ...
  return (
    <div className="p-6 space-y-5">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 h-8 text-xs rounded-md border transition-colors ${
                preview ? 'bg-gray-700 text-white border-gray-800' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
              }`}
            >
              {preview ? 'エディタ' : 'プレビュー'}
            </button>
            <Button variant="primary" onClick={() => setConfirmOpen(true)}>バージョンを発行</Button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-2 gap-5">
        {/* MOU text editor */}
        <div>
          <p className="text-xs font-medium text-gray-700 mb-2">MOU 本文（Markdown）</p>
          {preview ? (
            <div className="bg-white rounded-xl border border-gray-100 p-4 min-h-64 prose prose-sm max-w-none text-xs">
              <ReactMarkdown remarkPlugins={[remarkGfm]}>{mouText}</ReactMarkdown>
            </div>
          ) : (
            <textarea
              value={mouText}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <Button size="sm" variant="secondary" onClick={addItem}>+ 追加</Button>
          </div>
          <div className="space-y-2">
            {checklist.map((item, i) => (
              <div key={i} className="flex items-center gap-2 bg-white rounded-lg border border-gray-100 p-2.5">
                <input
                  type="text"
                  value={item.label}
      ...
      {confirmOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-40 p-4">
          <div className="bg-white rounded-xl shadow-xl p-5 w-full max-w-sm">
            <h3 className="text-base font-medium text-gray-900 mb-2">バージョンを発行しますか？</h3>
            <p className="text-sm text-gray-600 mb-1">新しいバージョンが発行されます。</p>
            <p className="text-xs text-gray-400 mb-5">既存の承認済みユーザーへの影響はありません。</p>
            <div className="flex gap-2 justify-end">
              <Button variant="secondary" onClick={() => setConfirmOpen(false)}>キャンセル</Button>
              <Button variant="primary" onClick={handleSave} loading={updateMou.isPending}>発行する</Button>
            </div>
          </div>
        </div>
      )}
