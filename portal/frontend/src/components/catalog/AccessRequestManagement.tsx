              ...
              {(data?.items ?? []).map((req) => (
                <tr key={req.agreement_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-xs text-gray-500">
                    {new Date(req.agreed_at).toLocaleString("ja-JP")}
                  </td>
                  <td className="px-4 py-2.5">
                    <Badge
                      variant={
                    ...
                    {req.status === "PENDING" && (
                      <div className="flex gap-1.5">
                        <Button
                          ...
                          variant="primary"
                          onClick={() =>
                            handleDecide(req.agreement_id, "approve")
                          }
                        ...
                        <Button
                          size="sm"
                          variant="danger"
                          onClick={() => setRejectTarget(req.agreement_id)}
                        >
                          却下
                        </Button>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-40 p-4">
          <div className="bg-white rounded-xl shadow-xl p-5 w-full max-w-sm">
            ...
            <div className="flex gap-2 justify-end mt-3">
              ...
              <Button
                variant="danger"
                onClick={() => handleDecide(rejectTarget, "reject")}
                loading={decide.isPending}
              >
                却下する
              </Button>
